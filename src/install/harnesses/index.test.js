...
describe('writeHarnessConfigs()', () => {
  ...
  it('completes without error for empty harness list', () => {
    assert.doesNotThrow(() => writeHarnessConfigs([], 'global', 3001, 3002, { homeDir: dir(), cwd: dir() }));
  });
  ...
  it('writes both config files when two harnesses are selected', () => {
    ...
    assert.ok(fs.existsSync(path.join(dir(), '.codex', 'config.json')), 'codex config should exist');
  });

  it('completes without error for unknown harness name', () => {
    assert.doesNotThrow(() => writeHarnessConfigs(['unknown-harness'], 'global', 3001, 3002, { homeDir: dir(), cwd: dir() }));
  });

  it('writes all four harness configs when all are selected', () => {
    writeHarnessConfigs(['claude', 'codex', 'cursor', 'windsurf'], 'global', 3001, 3002, { homeDir: dir(), cwd: dir() });
